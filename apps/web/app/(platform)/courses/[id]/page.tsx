...
const MOCK_BLOCKS = [
  ...
  { id: "b7", type: "quiz", question: "Quelle est la complexité temporelle de l'algorithme de tri rapide (quicksort) en moyenne ?", options: ["O(n)", "O(n log n)", "O(n²)", "O(log n)"], answer: 1, explanation: "Le tri rapide a une complexité moyenne de O(n log n), ce qui en fait l'un des algorithmes de tri les plus efficaces en pratique." },
  { id: "b8", type: "divider" },
...
function Block({ block, completed, quizState, onMarkDone, onSelectQuiz }: {
  ...
  return (
    <div className={`bg-gray-900 border rounded-xl p-5 transition-all ${completed ? "border-primary/30" : "border-white/10"}`}>
      {block.type === "heading" && (
        <h2 className="text-xl font-bold text-white">{block.content}</h2>
      )}
