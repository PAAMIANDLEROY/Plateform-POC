...
const roleLabel: Record<string, string> = {
  ...
  superuser: "Super Admin", public: "Visiteur",
};
...
export default function ProfilePage() {
  ...
  async function handleDeleteAccount() {
    if (!confirm("Supprimer définitivement votre compte ? Vos données seront anonymisées sous 30 jours. Cette action est irréversible.")) return;
    ...
    <div className="max-w-2xl mx-auto px-6 py-12">
      ...
      <div className="flex items-start gap-6 mb-10">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-extrabold text-white">{fullName || "Profil incomplet"}</h1>
        ...
        {!editing && (
          <button onClick={() => setEditing(true)} className="text-sm text-primary hover:text-primary-light font-medium transition-colors">
            Modifier
          </button>
        )}
      </div>
      ...
      {editing && (
        <form onSubmit={handleSave} className="bg-gray-950 border border-white/10 rounded-2xl p-7 flex flex-col gap-5 mb-8">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="button" onClick={() => { setEditing(false); setError(""); }} className="px-5 py-2.5 rounded-xl border border-white/10 text-gray-400 text-sm hover:text-white hover:border-white/30 transition-all">
              Annuler
            </button>
          </div>
        </form>
      ...
      <div className="bg-gray-950 border border-white/10 rounded-2xl p-6">
        ...
        <div className="space-y-3">
          <RgpdRow
            ...
            desc="Téléchargez l'intégralité de vos données en JSON (Art. 20 RGPD)"
            action={<button onClick={handleExport} className="text-xs border border-white/10 text-gray-300 px-4 py-2 rounded-lg hover:bg-white/5 transition-colors whitespace-nowrap">Exporter JSON</button>}
          />
          <RgpdRow
            title="Gérer mes consentements"
            desc="Analytics, tracking comportemental (Art. 21 RGPD)"
            action={<Link href="/privacy" className="text-xs text-primary hover:text-primary-light transition-colors whitespace-nowrap">Voir la politique →</Link>}
          />
          <RgpdRow
            title="Supprimer mon compte"
            desc="Anonymisation sous 30 jours (Art. 17 RGPD)"
            action={<button onClick={handleDeleteAccount} className="text-xs border border-danger/30 text-danger px-4 py-2 rounded-lg hover:bg-danger/10 transition-colors whitespace-nowrap">Supprimer</button>}
            danger
          />
        </div>
      </div>
